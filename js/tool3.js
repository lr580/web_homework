...
$(() => {
    ...
    // $.ajax({
    //     type: 'get',
    //     url: 'resources/lorem_ch.txt',
    //     async: true,
    //     success: (rel) => {
    //         // console.dir(rel.split('\n'));
    //     }
    // })
    // let x = [];
    // for (let i = 0; i < 26; ++i) {
...
var now_lorem_lan = 'ch';
var now_lorem_num = 20;
var now_para_num = 1;

function sele_lorem_lan() {
    let new_lorem_lan = $('#lorem_lan').val();
    // console.log(new_lorem_lan);
    // if (new_lorem_lan === now_lorem_lan) {
    //     return;
    // }
    now_lorem_lan = new_lorem_lan;
    $('#lorem_num_text').html(now_lorem_lan === 'ch' ? '汉字数量：' : '单词数量：');
    $('#word_max_text').html(now_lorem_lan === 'en' ? '单词最大长度：' : '&nbsp;');
    $('#word_max').css('display', now_lorem_lan === 'en' ? 'initial' : 'none');
    // $('#word_max_text').css('display', now_lorem_lan == 'ch' ? 'none' : 'inline-block');
}

function sele_lorem_num() {
    let new_lorem_num = $("#lorem_num").val();
    // console.log($("#lorem_num").val());
    if (!new_lorem_num) {
        $("#lorem_num").val('1');
        new_lorem_num = 1;
    }
    now_lorem_num = Math.max(1, Math.min(parseInt($("#lorem_num").attr('max')), Math.abs(parseInt(new_lorem_num))));
    $("#lorem_num").val(now_lorem_num);
    $('#para_num').attr('max', now_lorem_num);
    let new_para_num = $('#para_num').val();
    if (new_para_num > now_lorem_num) {
        new_para_num = now_lorem_num;
    }
    now_para_num = new_para_num;
    $('#para_num').val(now_para_num);

    //将来可以尝试重构代码，全盘使用tool_common.js
    //以上是未重构的代码
    //以下是部分重构的代码
    down_fix_number('word_max', now_lorem_num);
    down_fix_number('stn_min', now_lorem_num);
    // up_fix_number('stn_max', now_lorem_num, true);
    // up_fix_number('stn_max', Math.min(now_lorem_num, parseInt($('#stn_max').attr('min'))));
    down_fix_number('stn_max', now_lorem_num);
    up_fix_number('stn_max', parseInt($('#stn_min').val()));
    // down_fix_number('stn_max', now_lorem_num);

    //可能仍然存在一些细节问题
}

function sele_para_num() {
    //未重构的代码
    let new_para_num = $("#para_num").val();
    if (!new_para_num) {
        $("#para_num").val('1');
    ...
    now_para_num = Math.max(1, Math.min(parseInt($("#para_num").attr('max')), Math.abs(parseInt(new_para_num))));
    $("#para_num").val(now_para_num);
}

function sele_word_max() {
    get_and_fix_number('word_max');
}

function sele_stn_min() {
    let val = get_and_fix_number('stn_min');
    up_fix_number('stn_max', val);
    // up_fix_number('stn_max', Math.max(parseInt($('#stn_max').val()), val));
}

function sele_stn_max() {
    get_and_fix_number('stn_max');
}

function lorem_clip(e) {
    // let content = '1123';
    // console.log(content);
    let target = document.getElementById('lorem_res');
    // target.value = content;
    target.select();
    // let target2 = document.getElementById('clipboard');
    let x = document.execCommand('Copy');
    // console.log(x);
    // target2.select();
    let obj2 = $('#clip_butt');
    obj2.val('复制文本成功');
...
function lorem_word(mx) {
    let len = randint(1, mx);
    let res = '';
    for (let i = 0; i < len; ++i) {
        res += choice(lorem_en, false);
    }
    return res;
}

//返回二元组，对应字符及实际句长
function lorem_sentence(lan, min, max, wmx) {
    let len = randint(min, max);
    let res = '';
    if (lan == 'ch') {
        for (let i = 0; i < len; ++i) {
            res += choice(lorem_ch, false);
        }
        res += choice(lorem_ch_punc, false);
    } else {
        for (let i = 0, ie = len - 1; i < ie; ++i) {
            res += lorem_word(wmx) + ' ';
        }
        res += lorem_word(wmx) + choice(lorem_en_punc, false);
    }
    return [res, len];
}

function lorem_paragraph(lan, min, max, wmx, len) {
    let res = '';
    let left = len;
    while (left > 0) {
        let new_stn = lorem_sentence(lan, min, Math.min(left, max), wmx);
        res += new_stn[0];
        left -= new_stn[1];
        if (lan == 'en' && left) {
            res += ' ';
    ...
    // if (lan == 'ch' && (res[reslen - 1] == '，' || res[reslen - 1] == '；')) {
